"!@testing SRVB:ZAPI_ENTERPRISEPROJECT_MCA
...
CLASS ltc_CREATE IMPLEMENTATION.
  ...
  METHOD create.
    DATA:
      ls_business_data TYPE ZI_EnterpriseProjectTP_MCA,
    ...
    lo_client_proxy = /iwbep/cl_cp_factory_unit_tst=>create_v4_local_proxy(
                    EXPORTING
                      is_service_key     = VALUE #( service_id      = 'Z_ENTREPRISEPROJECT_MCA'
                                                    repository_id   = 'SRVD'
...
* Prepare business data
    ls_business_data = VALUE #(
              projectuuid            = '11112222333344445555666677778888'
              projectinternalid      = '1'
              project                = 'Project'
    ...
    lo_request = lo_client_proxy->create_resource_for_entity_set( 'A_ENTERPRISEPROJECT' )->create_request_for_create( ).
...
CLASS ltc_READ_ENTITY IMPLEMENTATION.
  ...
  METHOD read_entity.
    DATA:
      ls_entity_key    TYPE ZI_EnterpriseProjectTP_MCA,
      ls_business_data TYPE ZI_EnterpriseProjectTP_MCA,
    ...
    lo_resource = lo_client_proxy->create_resource_for_entity_set( 'A_ENTERPRISEPROJECT' )->navigate_with_key( ls_entity_key ).
...
CLASS ltc_READ_LIST IMPLEMENTATION.
  ...
  METHOD read_list.
    DATA:
      lt_business_data TYPE TABLE OF ZI_EnterpriseProjectTP_MCA,
      lo_client_proxy  TYPE REF TO /iwbep/if_cp_client_proxy,
...
* lt_range_PROJECTUUID TYPE RANGE OF /s4ppm/tv_entity_guid,
* lt_range_PROJECTINTERNALID TYPE RANGE OF ps_s4_proj_pspnr.
    ...
    lo_request = lo_client_proxy->create_resource_for_entity_set( 'A_ENTERPRISEPROJECT' )->create_request_for_read( ).

    " Create the filter tree
*lo_filter_factory = lo_request->create_filter_factory( ).
*
*lo_filter_node_1  = lo_filter_factory->create_by_range( iv_property_path     = 'PROJECTUUID'
*                                                        it_range             = lt_range_PROJECTUUID ).
*lo_filter_node_2  = lo_filter_factory->create_by_range( iv_property_path     = 'PROJECTINTERNALID'
*                                                        it_range             = lt_range_PROJECTINTERNALID ).
